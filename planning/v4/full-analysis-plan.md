...
## 8. 后续增强登记（行业/个股层进入时落地）

### 8.1 Chokepoint Theory（供应链瓶颈投资框架）— 借鉴 Serenity @aleabitoreddit

**来源**：X 博主 Serenity（前 WSB / AI 供应链分析师），2 年 225 倍，核心方法论 = 自下而上逆向工程识别不可替代物理瓶颈。

**核心思路**：
1. 从终端爆发需求（如 AI 算力、人形机器人）出发，逐层拆解产业链至最底层零组件/原材料
2. 识别"物理开关"（Chokepoint）：不可替代 + 供给极度集中（1-2 家垄断）+ 产能扩张周期长
3. 需求指数级增长时，瓶颈供应商利润弹性远超终端巨头（非线性受益）
4. 全球视野无视市值/地域限制——专注机构覆盖盲区
5. 投前用对抗式 AI 辩论验证（Devil's Advocate），挑战替代路径/物理极限

**融入计划（Wave 3 行业/个股层落地）**：

> 📐 **完整设计已落 `planning/v4/chokepoint-framework.md`**（四维判定标准/逆向工程拆解/6维评分/角色分配/市场分层/局限/A-B实证）。本节仅留摘要，细节以该文档为准。
> ✅ **角色分配已由 A/B 测试定稿（2026-06-09）**：新增专职「产业链瓶颈分析师」角色，而非融入现有研究员——A/B 实测专职角色拆解更深（下钻到材料/设备底层）、含替代路径评估、能收敛 top_chokepoint，显著优于融合方案（推翻了最初"增强不新增"的直觉判断）。

| 改动位置 | 内容 |
|---------|------|
| `v4-industry-researcher.md` | 分析维度新增「产业链瓶颈扫描」：本行业高增长叙事中，哪个环节是物理不可替代的？供给集中度如何？扩产周期？ |
| `v4-industry-bear.md` | 强化"替代路径反驳"：如果 bull 依赖不可替代假设，bear 须搜寻替代技术路径证据 |
| `v4-stock-scout.md` / Scout | 标的筛选新增 Chokepoint 评分维度：不可替代性 / 供给集中度 / 产能弹性 / 需求确定性 / 市场定价效率 |
| `v4-stock-bull.md` | 看多论点可引用"瓶颈溢价"逻辑——需求爆发×供给受限=利润弹性 |
| `v4-stock-bear.md` | 挑战"瓶颈溢价"——替代技术路径 / 产能扩张打破壁垒 / 需求证伪 |

**不在本期范围**：大类层（Wave 1/2）不涉及产业链拆解，该增强仅在 Wave 3 行业/个股层进入时落地。
