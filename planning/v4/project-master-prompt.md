...
**🌿 瓶颈选股链(自下而上进攻链, 2026-06-15 用户拍板把紫苏叶方法论融入架构后新增编排单元 `alpha:bottleneck:<industry>`)**:
现有权益深链是"自上而下配置链"(行业→配比→个股)。瓶颈选股链是与之互补的**自下而上进攻链**, 嵌在**行业层判定 go 之后、个股层深研之前**, 作"瓶颈驱动的选股漏斗"。**触发条件**: 行业研究部门判定某行业 go(景气)。**4 步流程(主 agent orchestrate)**:
1. **Step1 产业链拆解** `v4-industry-chokepoint`: 对 go 行业自下而上逆向工程, 定位最短木板瓶颈环节(四维) + discovery_level🟢未发现
2. **Step2 紫苏叶挖掘** `v4-alpha-hunter`: 对每个瓶颈环节用五因子模型纯演绎(禁研报), 找小市值+技术垄断+不可或缺+市场不知道的卡位标的
3. **Step3 verified 核实** `v4-data-desk`: 逐只联网核实卡位真实性(该公司是否真在该环节有产品/客户)+ verified 市值(卡小市值画像)+ 催化剂时间窗。**禁止把演绎当 verified**
4. **Step4 五因子复核** `v4-investor-critic` 6.14: 五因子是否同时满足 + 低关注度硬门槛 + verified_vs_inferred 诚实标注
→ 验证通过的紫苏叶进入个股研究部门 `stock:<code>` 走完整 8-step。

**主 agent orchestration 职责(为何必须主 agent 分发, 永久铁律)**: 一个行业产业链拆出 5-10 个瓶颈环节, 每个环节找 3-5 个候选, 每个候选要 verified 核实 = **N×M×K 扇出**, 单个 subagent 研究不过来。主 agent 必须当 **orchestrator**: 选 go 行业 → 分发 chokepoint → 收瓶颈环节后分批分发 alpha-hunter → 收候选后分批分发 data-desk 核实 → 收敛进个股流程。**禁止主 agent 图省事自己一把抓**(=草草敷衍, 违背 MECE 反偷懒铁律)。

> 与 `v4-market-scanner`(横向全市场财务硬筛, 逆向价值发现)**互补**: scanner=横向, 瓶颈选股链=纵向产业链, MECE 两条进攻路径。
...
2. **低关注度不是唯一门槛, 核心是"未被充分定价"(2026-06-15 用户纠正)** — 两条路径任一: (a) 低关注度紫苏叶(信息不对称) 或 (b) 被错杀的卡位龙头(预期差, 卡位未变但价格过度悲观, 如中际旭创 ¥88 错杀买入)。**不因高关注度/大市值一刀切排除龙头**, 目标是盈利不是教条只买小紫苏叶。但仍排除"价格已过度反映乐观的真高位"。
...
5. **两类目标画像**: (a) 紫苏叶=市值小+技术垄断(全球 CR3 极高/国产唯一)+不可或缺(BOM 占比可能小但断供整线停)+市场不知道(听不懂/归错类); (b) 错杀龙头=卡位/景气/ROIC 未变+估值历史低位或低于可比+有修复催化(不要求小市值/低关注度)。
