...
10. **🧭 信号+价格双标铁律(D0-8 用户反馈两次精炼后, 永久)**: action_plan 必须**同时给两件事**:
    ...
    - **💰 价格**(动多少时具体下单点位): 必须有**推导链**, 每个数字注明来源:
      · 目标价 = base case forward EPS × 目标 PE (对标谁/为什么这倍数)
      · 加仓位 = 公允价 × 安全边际 -10% 至 -15%
      · 减仓位 = bull case × -10% 至 -20% (反映"乐观假设已 price-in"程度)
      · 止损位 = bear case 上沿/下沿 (跌破即承认 bear 论证成立)
    **错误态(单纯锚定)**: ❌ "等回调到¥X再买"(没有推导,纯心理预期)
    **错误态(纯信号无价格)**: ❌ "扣非破5%加仓"(用户看盘时没法操作, 不知道几块买)
    **正确态**: ✅ "扣非连续2季破5%(信号触发) → 按 ¥40-45 加仓(¥45=base case forward EPS×45x 公允, ¥40=安全边际-10%)"
    用户反馈: "有价格数字是对的, 但价格数字要合理真实"——价格不是锚定, 是 valuation 推导的下单具体执行点位.
