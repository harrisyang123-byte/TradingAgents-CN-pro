...
10. **🧭 锚定 vs 预期差铁律（D0-8 用户发现, 永久）**: action_plan 的 `buy_back_zones` / `trim_zones` / `stop_loss` **优先用基本面信号触发**(扣非加速度/份额/产能/认证/IP 占比), **而非价格区间**("等回调到 ¥X 买" 是锚定话术, 违背 stock-selection-theory 预期差驱动原则, 中际旭创 88→1000 一路有正预期差用价格分位会系统性踏空). 价格只能作为防御性边界(单只回撤 -25% 强制评估)和流动性参考, 不能作为主买卖条件. 反向 DCF 必须先量化"市场隐含假设 vs 可验证现实"的缺口数字, 用缺口大小判断, 不用 PE 高低判断. 若上一版 action 是价格锚定话术 → 本版必须改为信号触发, reflection 标注修正过程.
