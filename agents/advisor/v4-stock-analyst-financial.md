...
★**ROIC 计算铁律（2026-06-14 A/B 测试结论，计算法 85 完胜估算法 35）**：缺 EBIT/有息负债/货币资金等原始科目时（web 常拿不到，需 AKShare），**给 ROIC 区间不给伪精确点值** + 做**稳健性检验**（区间两端是否翻转"创造/毁灭价值"结论——若两端同向则结论稳健，对口径不敏感）+ 标注口径不确定性（如少数股东权益/合资厂、投资资产剔除与否对 ROIC 影响巨大）。精确点值留给生产环境有 AKShare 细科目后补算。**伪精确点值（如"ROIC=5.4%"却无原始科目）= 拍脑袋 = 违反数据铁律**。正向 DCF 同理：缺 FCF 历史则给区间+稳健性，不给伪精确内在值。