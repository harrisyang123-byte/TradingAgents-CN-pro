            ...
            <div class="decision-flow">
              <div class="flow-step" v-if="selectedAdvice.macro_judge_verdict">
                ...
                <div v-if="flowOpen === 'l1'" class="flow-step-body">
                  ...
                  <div v-if="selectedAdvice.market_debate_history" class="debate-section">
                    <div class="debate-toggle" @click="toggleL1Debate()">
                      ▶ L1 辩论记录 ({{ selectedAdvice.market_debate_history.length }} 字符)
                    </div>
                    <div v-if="isL1DebateOpen" class="advice-text" v-html="renderMd(selectedAdvice.market_debate_history?.slice(0, 5000))"></div>
                  </div>
                </div>
              </div>
              ...
              <div class="flow-step" v-if="selectedAdvice.stock_judge_verdict || selectedAdvice.stock_candidates?.length">
                ...
                <div v-if="flowOpen === 'l2'" class="flow-step-body">
                  ...
                  <div v-if="selectedAdvice.stock_debate_history" class="debate-section mt-2">
                    <div class="debate-toggle" @click="toggleL2Debate()">
                      L2 辩论记录 ({{ selectedAdvice.stock_debate_history.length }} 字符)
                    </div>
                    <div v-if="isL2DebateOpen" class="advice-text" v-html="renderMd(selectedAdvice.stock_debate_history?.slice(0, 5000))"></div>
                  </div>
                </div>
              </div>
              ...
              <div class="flow-step">
                ...
                <div v-if="flowOpen === 'l3'" class="flow-step-body">
                  ...
                  <div v-if="selectedAdvice.debate_history" class="debate-section mt-2">
                    <div class="debate-toggle" @click="toggleL3Debate()">
                      L3 完整辩论记录 ({{ selectedAdvice.debate_history.length }} 字符)
                    </div>
                    <div v-if="isL3DebateOpen" class="advice-text" v-html="renderMd(selectedAdvice.debate_history?.slice(0, 8000))"></div>
                  </div>
                </div>
              </div>
              ...
              <div class="flow-step">
                ...
                <div v-if="flowOpen === 'l4'" class="flow-step-body">
                  <el-table
                    :data="sortedHistoryPrescription"
                    stripe
                    size="small"
                    style="width:100%"
                    row-class-name="rx-table-row"
                  >
                    ...
                    <el-table-column label="权重变化" width="130" align="center">
                      <template #default="{ row: r }">
                        <span>{{ (r.current_weight || 0).toFixed(1) }}%</span>
                        <span v-if="Math.abs((r.target_weight || 0) - (r.current_weight || 0)) > 0.05" style="margin:0 4px;color:#909399">→</span>
                        <span v-if="Math.abs((r.target_weight || 0) - (r.current_weight || 0)) > 0.05" :style="{color: (r.target_weight || 0) > (r.current_weight || 0) ? '#67c23a' : '#f56c6c'}">
                          {{ (r.target_weight || 0).toFixed(1) }}%
                        </span>
                      </template>
                    </el-table-column>
                    <el-table-column label="优先级" width="80" align="center">
                      <template #default="{ row: r }">
                        <el-tag v-if="r.priority === 'important'" type="danger" size="small">重要</el-tag>
                        <span v-else class="text-muted">一般</span>
                      </template>
                    </el-table-column>
                    <el-table-column label="时机" width="90" align="center">
                      <template #default="{ row: r }">
                        <el-tag v-if="r.timing === 'immediate'" type="danger" size="small" effect="plain">立即</el-tag>
                        <el-tag v-else-if="r.timing === 'conditional'" type="warning" size="small" effect="plain">条件</el-tag>
                        <span v-else class="text-muted">—</span>
                      </template>
                    </el-table-column>
                    <el-table-column label="建议价" width="100" align="center">
                      <template #default="{ row: r }">{{ r.suggested_price || '—' }}</template>
                    </el-table-column>
                    <el-table-column label="PnL" width="75" align="right">
                      <template #default="{ row: r }">
                        <span :style="{color: (r.pnl_pct || 0) >= 0 ? '#67c23a' : '#f56c6c'}">{{ (r.pnl_pct || 0).toFixed(1) }}%</span>
                      </template>
                    </el-table-column>
                    <el-table-column label="理由" min-width="200" show-overflow-tooltip>
                      <template #default="{ row: r }">{{ (r.reasoning || '').slice(0, 120) }}</template>
                    </el-table-column>
                    <el-table-column type="expand" width="30">
                      <template #default="{ row: r }">
                        <div style="padding:8px 16px;font-size:13px;line-height:1.7;color:#606266;">
                          <div v-if="r.reasoning"><strong>理由：</strong>{{ r.reasoning }}</div>
                          <div v-if="r.suggested_price"><strong>建议价格：</strong>{{ r.suggested_price }}</div>
                          <div v-if="r.capital_source"><strong>资金来源：</strong>{{ r.capital_source }}</div>
                        </div>
                      </template>
                    </el-table-column>
                  </el-table>
...
function actionTagType(a: string) {
  ...
  if (act === 'buy' || act === 'add') return 'success'
  if (act === 'sell' || act === 'reduce') return 'danger'
  return 'info'
}

function actionLabel(a: string) {
  const map: Record<string, string> = { buy: '买入', add: '加仓', reduce: '减仓', sell: '卖出', hold: '持有' }
  return map[a?.toLowerCase() || ''] || a || '持有'
}
...
.market-signals-bar { display: flex; gap: 16px; padding: 6px 14px; background: #fef7e0; border-radius: 4px; margin-bottom: 12px; font-size: 12px; color: #8c6d1f; }
