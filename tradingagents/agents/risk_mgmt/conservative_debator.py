...
def create_conservative_debator(llm):
    def conservative_node(state) -> dict:
        ...
        # 📊 记录输入数据长度
        logger.info(f"📊 [Conservative Analyst] 输入数据长度统计:")
        ...
        total_length = (len(market_research_report) + len(sentiment_report) +
                       ...
                       len(current_aggressive_response) + len(current_neutral_response))
...
通过质疑他们的乐观态度并强调他们可能忽视的潜在下行风险来参与讨论。解决他们的每个反驳点，展示为什么保守立场最终是公司资产最安全的道路。专注于辩论和批评他们的论点，证明低风险策略相对于他们方法的优势。请用中文以对话方式输出，就像您在说话一样，不使用任何特殊格式。"""
        ...
        logger.info(f"⏱️ [Conservative Analyst] 开始调用LLM...")
        llm_start_time = time.time()

        response = llm.invoke(prompt)

        llm_elapsed = time.time() - llm_start_time
        logger.info(f"⏱️ [Conservative Analyst] LLM调用完成，耗时: {llm_elapsed:.2f}秒")
