...
def create_aggressive_debator(llm):
    def aggressive_node(state) -> dict:
        ...
        # 📊 记录输入数据长度
        logger.info(f"📊 [Aggressive Analyst] 输入数据长度统计:")
        ...
        total_length = (len(market_research_report) + len(sentiment_report) +
                       ...
                       len(current_conservative_response) + len(current_neutral_response))
...
以下是当前对话历史：{history} 以下是保守分析师的最后论点：{current_conservative_response} 以下是中性分析师的最后论点：{current_neutral_response}。如果其他观点没有回应，请不要虚构，只需提出您的观点。

积极参与，解决提出的任何具体担忧，反驳他们逻辑中的弱点，并断言承担风险的好处以超越市场常规。专注于辩论和说服，而不仅仅是呈现数据。挑战每个反驳点，强调为什么高风险方法是最优的。请用中文以对话方式输出，就像您在说话一样，不使用任何特殊格式。"""

        logger.info(f"⏱️ [Aggressive Analyst] 开始调用LLM...")
        import time
        ...
        logger.info(f"⏱️ [Aggressive Analyst] LLM调用完成，耗时: {llm_elapsed:.2f}秒")
