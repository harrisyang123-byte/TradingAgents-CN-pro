...
B（反思）与 C（反骑墙 + 源冲突接地）**本质是全局通用能力**（同 §5.8 data-desk 的「通用能力层」定位），对 v4 全部 4 个研判型角色普适——但**本期落地只收窄到大类层**，先验证闭环：

| Agent | B 反思 | C 反骑墙 | 本期落地 |
|-------|:---:|:---:|:---:|
| `v4-asset-director`（大类） | ✅ | ✅ | ✅ 本期 |
| `v4-industry-director`（行业） | ✅ | ✅ | ⏭ 下一阶段 |
| `v4-stock-director`（个股） | ✅ | ✅ | ⏭ 下一阶段 |
| `v4-allocation-director`（配比） | ✅ | 仅反思 | ⏭ 下一阶段 |

反思地基已全局通用（`archive_v4.py` 不分层归档、`write_unit` 覆盖前留底对所有单元生效、`_conclusion()` 兼容全部结论字段、「write 前落盘仍是旧版」时序巧合对 4 个 director 同样成立），下一阶段只需把同一段 reflection 节 + 反骑墙条款复制到另外 3 个 director、源冲突规则补到行业/个股分析师，**零新基建、低成本铺开**。约束：下一阶段沿用本期定稿的 `reflection` 字段结构与措辞，不另立格式。详见 `.kiro/specs/v4/design.md §5.9.5`。**本 change 实现范围与其余 spec 不变，本节仅定性与排期。**
