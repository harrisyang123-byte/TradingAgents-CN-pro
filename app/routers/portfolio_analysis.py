...
async def start_l1_plan(current_user: dict = Depends(get_current_user)):
    ...
    from tradingagents.utils.llm_factory import create_llm_by_provider

    executor = ThreadPoolExecutor(max_workers=1, thread_name_prefix="l1_plan")
...
async def execute_l2_l4(
    ...
    executor = ThreadPoolExecutor(max_workers=1, thread_name_prefix="l2l4_exec")
        ...
        try:
            async def _exec():
                ...
                result = advisor.propagate_advice(
                    ...
                    progress_callback=progress_cb,
                    selected_industries=req.selected_industries,
                )
                ...
                try:
                    ...
                    await db2["analysis_reports"].insert_one({
                        ...
                        "stock_symbol": f"portfolio_{req.task_id}",
                        "stock_name": f"组合建议 {completed_at[:10]}",
                        "summary": (result.get("cio_verdict", "") or "")[:500],
                        "recommendation": f"覆盖 {len(req.selected_industries)} 个行业, {len(result.get('prescription', []))} 条处方",
                ...
                try:
                    for ind_name in req.selected_industries:
                        await db2["industry_coverage"].update_one(
                            {"user_id": user_id, "industry_name": ind_name},
                            {"$set": {
                                "status": "completed",
            ...
            async def _mark_failed_2():
                db2 = get_mongo_db()
                await db2["portfolio_advice"].update_one(
                    {"advice_id": req.task_id},
                    {"$set": {"status": "FAILED", "error": str(e)[:500], "updated_at": datetime.utcnow().isoformat()}},
                )
            loop.run_until_complete(_mark_failed_2())
        finally:
            loop.close()

    executor.submit(_run_l2_l4)
    return {"task_id": execute_id, "status": "running"}
...
async def get_task_status(
        ...
        try:
            ...
            if data:
                ...
                if progress.get("status") == "completed":
                    doc = await db["portfolio_advice"].find_one({"advice_id": task_id.replace("_exec", "")})
                    if doc and doc.get("status") in ("COMPLETED", "L1_COMPLETED"):
                        return {
    ...
    # 回退到 MongoDB
    base_id = task_id.replace("_exec", "")
    doc = await db["portfolio_advice"].find_one({"advice_id": base_id})
    if not doc:
    ...
    if status == "failed":
        return {"status": "failed", "error": doc.get("error", "未知错误")}

    return {
        "status": doc.get("status", "UNKNOWN").lower(),
        "current_step": doc.get("current_step", "分析中..."),
        "progress": 30 if status == "l1_completed" else 10,
    }
