...
> 一切围绕「**分析单元**」（某大类 / 某行业 / 某个股 / 某配比）展开；每个单元独立触发、独立缓存、独立有新鲜度。**取数集中、辩论只读**：一个共享的「数据采集台」带 web 工具，**只把一层结构化的跨单元公共指标（7维度~22个数：利率/物价/信用/汇率/海外/风险/大宗）取一次全局共享，其余大量数据逐单元深取、按需多次**；各「分析部门」（多空 3 轮辩论 + 总监拍板）只读输入包做研判；产物以统一信封覆盖式落盘，既是 git 传输载体，又供前端三层 Tab 展示（默认走静态快照、Mongo 可选）。

---

## 1. 分层总览（6 横层 × 3 纵深）

> **前端展示与现在完全一致，你永远不会看到一堆 JSON。** 落盘的单元信封 / 静态快照只是**数据**，看页面的是同一套 Vue 组件（`V4Overview.vue` + `AssetCard`/`IndustryTable`/`PlanCard`…），它 fetch 这些 JSON 后渲染成三层 Tab 的卡片与表格。`VITE_STATIC_SNAPSHOT=1` 只切换**数据来源**（本地快照文件 ↔ `/api` 读 Mongo），两者返回结构同构、**渲染层一字不改**——展示效果与现状的组合总揽一模一样。
...
flowchart TB
    ...
    subgraph L3["③ 通用能力层 Shared Capabilities（各业务层共用，★本次优化核心）"]
        DESK["🛰 数据采集台 v4-data-desk<br/>（唯一带 web 工具的 Agent）<br/>薄层全局公共指标取一次 + 逐单元深取（按需多次）"]
...
```mermaid
flowchart LR
    subgraph DESK["🛰 v4-data-desk（带 web 工具，按单元调用）"]
        A["档A 薄层全局公共指标<br/>run 起手取一次（仅十来个数）"]
        B["档B 单元级深取<br/>分析某单元时按需、可多次"]
    end
    A -->|7维度公共指标: 利率/物价景气/信用流动性/汇率/海外/风险/大宗<br/>每项带 verified+来源URL| MAC["inputs/data_macro.json"]
    B -->|权益:行业景气/估值 ·固收:收益率曲线/信用利差<br/>大宗:库存/期货升贴水 ·个股:财报/资金流| PACK["inputs/<单元>.json"]
    MAC --> ALL["全部 7 大类 + 行业 + 个股 共读同一份（一致性）"]
    PACK --> DEP["仅对应单元的部门读（深度）"]
```

- **档 A（run 级，薄薄一层但结构化）**：**只取那些对所有单元都一样的公共指标**（LPR 就是一个数，权益和固收用的必须是同一个）。按**自上而下 7 个维度结构化覆盖**（~22 个数，仍远薄于档 B），对齐真实多资产配置台的公共面板——既覆盖「信用驱动的 A 股」，也覆盖 v4 大类层自带的「海外敞口」：

  | 维度 | 指标 | 服务对象 |
  |------|------|----------|
  | 货币/利率 | 1Y/5Y LPR、7天逆回购、10Y国债、期限利差(10Y-2Y) | 全单元 |
  | 物价/景气 | CPI、PPI、制造业PMI、非制造业PMI | 全单元 |
  | 信用/流动性 | 社融存量同比、M2同比、两融余额 | A股权益/行业（信用脉冲） |
  | 汇率 | USDCNY、美元指数DXY | 全单元 + 海外敞口 |
  | 跨市场/海外敞口 | 美10Y国债、联邦基金利率、标普500、纳斯达克 | 大类层海外敞口 |
  | 风险情绪 | VIX | 全单元（risk-on/off） |
  | 大宗/避险 | 布伦特原油、COMEX黄金、LME铜 | 大宗/贵金属/周期 |

  取一次写 `data_macro.json`，7 大类/所有行业/所有个股**共读这一份**——共享是为了**约束链一致性**（同源同指纹由构造保证），**不是"批量把数据抓完"**。跨市场/海外指标只服务大类层海外敞口，A 股行业/个股层不引用。北向资金自 2024-08 停止每日披露，已用两融余额替代。
- **档 B（单元级，重活在此）**：**真正的大量取数逐单元、深度、按需进行**——分析权益时取权益那一大坨（行业景气/估值），分析固收时再取固收那一大坨（收益率曲线/信用利差），个股则取财报/资金流。**一个单元内部 data-desk 可多次取数**（财报一次、估值一次、资金流一次），取到够该单元辩论为止。这与「逐个大类深研、不一锅烩」对齐：**深研到哪个单元，就深取哪个单元的数据。**
- **心智模型**：data-desk 是一个**带 web 工具的"取数角色/能力"，每分析一个单元就为它深取一轮**，**不是**开跑前把全宇宙数据一次抓完的批处理。
