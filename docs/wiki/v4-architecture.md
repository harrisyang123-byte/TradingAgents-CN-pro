...
> 一切围绕「**分析单元**」（某大类 / 某行业 / 某个股 / 某配比）展开；每个单元独立触发、独立缓存、独立有新鲜度。**取数集中、辩论只读**：一个共享的「数据采集台」带 web 工具，**只把十来个跨单元公共指标（LPR/CPI/汇率…）取一次全局共享，其余大量数据逐单元深取、按需多次**；各「分析部门」（多空 3 轮辩论 + 总监拍板）只读输入包做研判；产物以统一信封覆盖式落盘，既是 git 传输载体，又供前端三层 Tab 展示（默认走静态快照、Mongo 可选）。
...
- **档 A（run 级，薄薄一层）**：**只取那十来个对所有单元都一样的公共指标**（LPR 就是一个数，权益和固收用的必须是同一个）。取一次写 `data_macro.json`，7 大类/所有行业/所有个股**共读这一份**——共享它是为了**约束链一致性**（同源同指纹由构造保证），**不是为了"批量把数据抓完"**。
